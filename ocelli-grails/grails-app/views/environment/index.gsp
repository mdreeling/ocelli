		...
		<div id="list-environment" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="envCode" title="${message(code: 'environment.envCode.label', default: 'Env Code')}" />
					
						<g:sortableColumn property="envDescription" title="${message(code: 'environment.envDescription.label', default: 'Env Description')}" />
				...
				<g:each in="${environmentInstanceList}" status="i" var="environmentInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${environmentInstance.id}">${fieldValue(bean: environmentInstance, field: "envCode")}</g:link></td>
					
						<td>${fieldValue(bean: environmentInstance, field: "envDescription")}</td>
